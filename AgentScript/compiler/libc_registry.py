...
Each entry maps an AgentScript call target of the form `c.<funcName>` to a
tuple:

    (return_type, [param_type, …], var_args)

where each type is a spec-compliant AgentScript alias declared in
ascc.py's `llvm_type_for`. Every name carries width + signedness, or an
...
compiler reads it on demand: when an AgentScript program calls `c.fname`,
the compiler looks up the signature, declares `LLVMfname` as an external,
and emits a direct call. There is no AgentScript-side trampoline, so the
call cost is the same as a native C call to the same libc function.

camelCase AS-facing aliases
---------------------------
AgentScript source code, per spec §5, restricts identifier punctuation to
...
AgentScript programs invoke these targets without violating the lexical
rules, this module publishes `AS_FACING_ALIASES`, a dict mapping
...
`resolve_c_symbol(as_name)` before consulting `ALL_FUNCTIONS`.
...
the camelCase aliases are purely additive. New AgentScript source should
...
MATH = dict([
    ...
    # in ascc._emit_run and lowered to LLVM fcmp / bit-twiddling instead of
...
# the AS-side type aliases CComplexDouble / CComplexFloat are not currently
...
# alias. AgentScript source code uses the keys on the left; the registry
# itself still keys signatures by the C symbol on the right. The compiler
# resolves AS-facing names through `resolve_c_symbol()` below.
AS_FACING_ALIASES = {
    # ---- <stdio.h> ----
    "getsSafe":                    "gets_s",
    # ---- <stdlib.h> ----
...
def resolve_c_symbol(as_name: str) -> str:
    """Translate an AgentScript-facing call target to its C symbol.

    AgentScript source code is forbidden by spec §5 from using `_` in
    identifiers, so libc functions like `aligned_alloc` cannot be
    written directly as `c.aligned_alloc`. Instead, AS source writes
    ...
    If `as_name` is already a legal C symbol (i.e. has no AS-facing
    ...
    """
    return AS_FACING_ALIASES.get(as_name, as_name)
...
# We expose `c.errnoGet` as the canonical AgentScript accessor.
...
if __name__ == "__main__":
    ...
    print(f"AgentScript libc registry — {total} functions across {len(summary)} headers:")
