...
AgentScript line tape directly and checks source-level laws that do not require
...
def lint_line_shape(program: ProgramFacts, diags: List[Diagnostic]) -> None:
    for line in program.lines:
        ...
        if verb not in KNOWN_VERBS:
            add_diag(diags, "error", "unknownVerb", line, f"unknown AgentScript verb `{verb}`")
            continue
        ...
        if any(("{" in token.text or "}" in token.text) for token in line.tokens if not token.quoted):
            add_diag(diags, "warning", "nestedSyntax", line, "brace syntax conflicts with AgentScript's flat line-tape model")
        ...
        for arg_index in vague_positions:
            ...
            if arg in BAD_NAMES:
                add_diag(
                    ...
                    "vagueName",
                    line,
                    f"`{arg}` is too vague for AgentScript context-maxxing",
...
def collect_paths(paths: Sequence[str]) -> List[Path]:
    ...
    for raw_path in paths:
        path = Path(raw_path)
        if path.is_dir():
            collected.extend(sorted(path.rglob("*.as")))
        elif path.is_file():
            collected.append(path)
        else:
            matches = sorted(Path().glob(raw_path))
            collected.extend(match for match in matches if match.is_file() and match.suffix == ".as")
    return collected
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not paths:
        print("aslint: no .as files matched", file=sys.stderr)
        return 2
