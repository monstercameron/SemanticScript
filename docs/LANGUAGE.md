# SemanticScript Language Guide

> The complete reference for the SemanticScript language: the row model, the
> grammar, the per-kind predicate vocabulary, and every entity kind, with worked
> examples. This is the deep guide — for a 5‑minute "install it and run
> something" walkthrough, start with
> [docs/getting-started.md](docs/getting-started.md); for the project overview,
> see the [README](README.md).

SemanticScript programs are flat rows of the shape
`<subject> <predicate> <payload>`. There are no nested blocks and no verb
families to special-case — one grammar that agents and tools can grep, patch,
and validate uniformly. Every program is still bound by the same contract:
**effects, failures, cleanup, memory, time, and authority are source data**, not
comments. Authority is expressed through the `uses` + `grants` + `effect` trio;
return arity is typed; cleanup is registered explicitly; metadata attaches
universally.

This guide is the normative description of that grammar. The toolchain
(`semanticscript`) lexes, parses, lints, lowers to LLVM IR, and JIT-runs or
builds native executables from exactly the forms described here. Where a section
references a worked program, the corresponding file lives under
[`examples/`](examples/) or [`apps/`](apps/) and is exercised by the test suite.
