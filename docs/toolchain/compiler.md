...
requests. That worker uses the same real sentence-transformer embeddings as the
CLI docs indexer, so install `requirements-docs.txt` when using indexed docs.
`docs_search` does not start the background worker unless `watch` is explicitly
true, and model downloads require an explicit allow flag.
