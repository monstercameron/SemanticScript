...
> **AgentScript**: a compiled application language for agent-authored software, competing with Node/Python at the app layer, but deploying more like Go. Its source is a flat, line-oriented, high-context program tape where every executable line is an atomic English-explicit semantic record.

The point is not to make code short. The point is to make code **locally understandable inside an agent attention window**.

AgentScript is agent-first, not human-first. Human readability is useful, but it is not the design center.

A normal language compresses meaning into nesting, scope, syntax, conventions, and runtime behavior. AgentScript should do the opposite:
...
AgentScript is not a human-friendly scripting language.
...
Node/Python are good because they are fast to write. AgentScript should be good because it is **hard to misunderstand**.
...
So AgentScript should be long, explicit, regular, and brutally inspectable.

Verbosity is not a failure mode here. Verbosity is the representation strategy.

AgentScript practices **context-maxxing**: it spends tokens to keep local meaning alive.
...
```text
1. AgentScript maximizes recoverable context.
...
Bad AgentScript should look obviously bad.

Good AgentScript should make the next correct edit easy.
...
AgentScript should make it obvious when data crosses from untrusted to trusted, raw to sanitized, internal to public, local to remote, or dependency-owned to application-owned.
...
AgentScript treats abstraction as more meaning per referenced node.
...
AgentScript should not have casual helpers.
...
AgentScript source is optimized for partial retrieval.
...
This is why AgentScript prefers explicit local repetition over distant inference.
...
AgentScript should reject or warn on context collapse:
...
Executable AgentScript is not natural language.
...
But the AgentScript version exposes:
...
AgentScript should be **monofile-first**, not monofile-only.
...
For Node/Python-style ecosystems, this is critical. AgentScript packages should ship both:
...
This is how AgentScript avoids Node's worst debugging traps.
...
This is where AgentScript becomes much safer than Node/Python service code.
...
AgentScript should ship with tools designed around agent workflows.
...
Checks source against the AgentScript constitution.
...
AgentScript should deliberately reject many things that Node/Python allow.
...
AgentScript should not be "a simpler programming language."

It should be:

> An agent-first application language where source code is a controlled English command tape made of atomic contextual lines.
