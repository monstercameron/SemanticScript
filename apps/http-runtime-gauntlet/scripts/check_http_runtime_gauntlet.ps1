...
# This script lives in apps/http-runtime-gauntlet/scripts/, so the repo root climb
# is three levels (..\..\..). The compile target is the project-mode
# build tape (build.sem) in the parent directory; semsc inlines main.sem.
$RepoRoot = Resolve-Path (Join-Path $ScriptDir "..\..\..")
$AppDir = Resolve-Path (Join-Path $ScriptDir "..")
$SourcePath = Join-Path $AppDir "build.sem"
$ExePath = Join-Path $ScriptDir "http_runtime_gauntlet.exe"
...
function Wait-GauntletServer {
    for ($attempt = 0; $attempt -lt 80; $attempt += 1) {
        try {
            ...
            Assert-GauntletHeader -Response $health -Name "X-Gauntlet-App" -ExpectedValue "http-runtime-gauntlet"
            return
        } catch {
...
try {
    ...
    Write-Host "HTTP Runtime Gauntlet curl smoke passed"
} finally {
