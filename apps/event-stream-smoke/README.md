# event-stream-smoke (EAV port)

Full 1:1 EAV-Steps port of `apps/event-stream-smoke` (the v0.1 SemanticScript
...
- **12 capabilities**, matching the original (the original's resource-first
  `capability NAME RESOURCE ACTION` rows become EAV `capability` entities with
...
- **17 task entities + 3 call entities** — the original's inline async
  ...
  (the §20 split). Every `event.*` call is a `task` (EAV async = `start`/`join`,
...
- `storage`: 13 → 12. The only dropped storage is `requestCancellationToken`,
  used solely by the v0.1 `cancelOn` rows; EAV tasks carry no per-task cancel
...
- The v0.1 `console.writeLine` error path in each listener is dropped:
  `console.writeLine` is infallible (void) in EAV, so the `bind error` /
  `writeFailed` label disappears.
- `start CALL` / `await CALL` / `bind value X T CALL` → an EAV `task` with
