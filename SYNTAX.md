...
| `routeTimeoutOptOut SERVER PATH "rationale"` / `routeMiddlewareOptOut SERVER PATH "rationale"` | Per-path explicit opt-out from the cross-cutting `routeTimeout` / `routeMiddleware` contract. Accepted by semsc as metadata (stored under server hard_metadata); read by `semlint2` SS3604 `routeCoverageDrift` to suppress the coverage warning on routes that intentionally skip the contract. The rationale string makes the omission a declared choice rather than a quiet gap. | Impl'd |
| `pinsNullBodyFailurePath OP "rationale"` | Explicit opt-in to the native HTTP adapter's null-body 500 contract on a single operation. Replaces the legacy stringly-typed `null-body failure path` marker that used to live inside `warning OP "..."` text. `semlint2` SS3603 treats the verb as the canonical opt-out; SS3606 requires the rationale string to be non-empty. The legacy marker is still honored for one deprecation cycle and trips SS3605 `legacyNullBodyMarker` when load-bearing. | Impl'd |
| `responseBodyForwarder OP bodyArgName` | Declares that an operation forwards its `bodyArgName` input straight into an `http.response*` writer (or another declared forwarder). Replaces the linter's `arg X body body` arg-name-shape inference. `semlint2` SS3603's transitive walk only follows wrappers that carry this verb; SS3607 `forwarderDeclarationNotHonored` fires when the declaration is not backed by a real forwarding `arg` line. | Impl'd |
| `rationale CALL "text"` | Operation-body counterpart to the `# rationale:` typed comment; attaches a rationale to a specific call name so diagnostics can cite it by lookup rather than by comment proximity. `semlint2` surfaces these via `narrative_citations_for_operation`. SS3608 `rationaleReferencesUnknownCall` / `rationaleMissingText` enforces that the verb names a real call and carries non-empty text. | Impl'd |
