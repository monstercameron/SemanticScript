"""
feature_coverage.py — exercise individual SemanticScript language features through
the SemanticScript-written compiler (bootstrap_general.sscript) and report pass/fail.

Each test program in sem/feature_tests/ has a `# expect:` metadata header
...
Each test is compiled by semsc.py against bootstrap_general.sscript into IR,
...
BOOTSTRAP_GENERAL = ROOT / "bootstrap" / "bootstrap_general.sscript"
...
def compile_via_sem_compiler(source_path: Path) -> tuple[bool, str, str]:
    """Run bootstrap_general.sscript via the Python compiler with SEMANTIC_SCRIPT_INPUT set
    to source_path. Returns (success, ir_text, stderr)."""
    env = os.environ.copy()
...
def evaluate_test(source_path: Path) -> tuple[str, str]:
    ...
    exe_path = BUILD_DIR / (source_path.stem + ".exe")

    ok, ir_text, stderr = compile_via_sem_compiler(source_path)
    if not ok or not ir_text:
        if xfail:
            return ("XFAIL", f"sem-compile failed: {stderr.strip()[:200]}")
        return ("FAIL", f"sem-compile failed: {stderr.strip()[:200]}")
...
def main():
    ...
    test_files = sorted(TEST_DIR.glob("*.sscript"))
    if args.filter:
    ...
    for source_path in test_files:
        status, detail = evaluate_test(source_path)
        counts[status] += 1
        symbol = {"PASS": "[OK ]", "FAIL": "[FAIL]",
                  "XFAIL": "[xfail]", "XPASS": "[XPASS]"}[status]
        print(f"  {symbol} {source_path.name:<40} {detail if args.verbose or status in ('FAIL','XPASS') else ''}")
        if status in ("FAIL", "XPASS"):
            failures.append((source_path.name, status, detail))
