...
def test_parser_syntax_cutover_rows():
    src = "\n".join([
        ...
        "error MainError",
        "errorCase MainError NotFound I32",
        "storage module mutable searchAttempts I64 0",
        "operation main",
        "input operation main needle I64",
        "output operation main Result I64 MainError",
        "memory main heap no",
        ...
        "argument addCall left I64 low",
        "argument addCall right I64 needle",
        "run addCall",
        "bind value nextLow I64 addCall",
    ...
    check("parser cutover: typed argument row recorded",
          any(verb == "argument" and args == ["addCall", "left", "I64", "low"]
...
def test_compile_new_syntax_rows_to_ir():
    src = "\n".join([
        ...
        "error MainError",
        "errorCase MainError Failed I32",
        "storage module mutable searchAttempts I64 0",
        "operation helper",
...
def _strict_http_route_source(
    ...
    if handler_body is None:
        handler_body = [
            ...
            "argument wrapperCall status CSignedInt32 okStatus",
            "argument wrapperCall body CNullTerminatedByteString okBody",
            "run wrapperCall",
            "bind value responseStatus CSignedInt32 wrapperCall",
    ...
    return "\n".join([
        ...
        *wrapper_forwarder_lines,
        ...
        "argument writeCall status CSignedInt32 status",
        "argument writeCall body CNullTerminatedByteString body",
        "run writeCall",
        "bind ok writeStatus CSignedInt32 writeCall",
        "bind error writeCallError CSignedInt32 writeCall",
...
def test_strict_web_contracts_reject_nullable_header_response_body():
    handler_body = [
        "storage module immutable okStatus CSignedInt32 200",
        "storage module immutable tokenHeaderName CNullTerminatedByteString \"X-Token\"",
        ...
        "run headerReadCall",
        "bind value maybeToken CNullTerminatedByteString headerReadCall",
        "call writeCall http.responseText",
        "argument writeCall response HttpResponse response",
        "argument writeCall status CSignedInt32 okStatus",
        "argument writeCall body CNullTerminatedByteString maybeToken",
        "run writeCall",
        "bind value responseStatus CSignedInt32 writeCall",
        "return value responseStatus",
    ]
...
def test_strict_rejects_plain_run_for_fallible_heap_allocation():
    src = "\n".join([
        ...
        "async main no",
        "label start",
        "storage module immutable allocationSize CByteCount 8",
        "call mallocCall c.malloc",
        "argument mallocCall size CByteCount allocationSize",
        "run mallocCall",
        "return ok noResult",
    ])
    proc = run_semsc_source(src, "--parse-only", "--strict", "--quiet")
...
def test_strict_accepts_complete_legacy_checked_fallible_call_pattern():
    src = "\n".join([
        ...
        "argument mallocCall size CByteCount allocationSize",
        "run mallocCall",
        "bind ok heapBuffer COpaqueMemoryAddress mallocCall",
...
def test_strict_rejects_missing_status_for_fallible_http_response_write():
    src = "\n".join([
        ...
        "label start",
        "storage module immutable okStatus CSignedInt32 200",
        "storage module immutable bodyText CNullTerminatedByteString \"ok\"",
        "storage module immutable plainType CNullTerminatedByteString \"text/plain\"",
        "call writeResponseCall http.responseText",
        "argument writeResponseCall response HttpResponse response",
        "argument writeResponseCall status CSignedInt32 okStatus",
        "argument writeResponseCall body CNullTerminatedByteString bodyText",
...
def test_strict_executable_rejects_heap_allocation_without_oom_branch():
    src = "\n".join([
        ...
        "argument allocationCall size CByteCount allocationSize",
        "run allocationCall",
        "bind ok heapBuffer COpaqueMemoryAddress allocationCall",
        "return ok noResult",
    ])
    proc = run_semsc_source(src, "--parse-only", "--quiet")
...
def test_strict_executable_rejects_double_heap_free():
    src = "\n".join([
        ...
        "argument firstFreeCall ptr COpaqueMemoryAddress heapBuffer",
        "run firstFreeCall",
        "call secondFreeCall c.free",
        "argument secondFreeCall ptr COpaqueMemoryAddress heapBuffer",
        "run secondFreeCall",
        "return ok noResult",
        "label allocationFailed",
        "return error allocationError",
...
def test_compile_i32_comparison_to_i32_ir():
    source = "\n".join([
        ...
        "invariant operation main \"math.lessThanCSignedInt32 lowers to an i32 signed compare\"",
        "label startMain",
...
def test_compile_rejects_implicit_i32_to_i64_math():
    source = "\n".join([
        ...
        "storage module immutable successExit ExitCode 0",
        "storage module immutable leftStatus CSignedInt32 1",
        "storage module immutable rightStatus CSignedInt32 1",
        "call statusCheckCall math.equalI64",
        "argument statusCheckCall left CSignedInt32 leftStatus",
        "argument statusCheckCall right I64 rightStatus",
        "run statusCheckCall",
        "return value successExit",
    ])
    try:
...
def test_compile_explicit_i32_to_i64_conversion_lowers_to_sext():
    source = "\n".join([
        ...
        "storage module immutable successExit ExitCode 0",
        "storage module immutable sourceValue CSignedInt32 -1",
        "call widenCall math.signExtendCSignedInt32ToCSignedInt64",
        "argument widenCall inputValue CSignedInt32 sourceValue",
        "run widenCall",
        "bind value widenedValue CSignedInt64 widenCall",
        "return value successExit",
    ])
    prog = semsc.parse(source)
    cg = semsc.Codegen(prog)
...
def test_compile_pointer_load_byte_sign_extends_to_i32():
    source = "\n".join([
        ...
        "memory main heap no",
        "async main no",
        "purpose operation main \"pointer.loadByte returns a signed CSignedInt32 byte value\"",
        "storage module immutable successExit ExitCode 0",
        "storage module immutable text CNullTerminatedByteString \"A\"",
        "storage module immutable zeroOffset CByteCount 0",
        "call loadByteCall pointer.loadByte",
        "argument loadByteCall buffer CNullTerminatedByteString text",
        "argument loadByteCall offset CByteCount zeroOffset",
        "run loadByteCall",
        "bind value loadedByte CSignedInt32 loadByteCall",
        "return value successExit",
    ])
    prog = semsc.parse(source)
    cg = semsc.Codegen(prog)
...
def test_html_template_simple_jit_output():
    source = "\n".join([
        ...
        "ignore value source writeGreetingCall type CSignedInt32",
...
def test_html_template_edge_output_repeated_adjacent_and_blank_lines():
    source = "\n".join([
        ...
        "run writeEdgeCall",
        "ignore value source writeEdgeCall type CSignedInt32",
        "return value successCode",
    ])
    proc = run_semsc_source(source, "--run", "--quiet", suffix=".sem")
    expected = (
...
def test_html_template_raw_style_and_script_do_not_hydrate_braces():
    source = "\n".join([
        ...
        "run writeRawCall",
        "ignore value source writeRawCall type CSignedInt32",
        "return value successCode",
    ])
    proc = run_semsc_source(source, "--run", "--quiet", suffix=".sem")
    expected = (
...
def test_html_template_escapes_html_text_by_sink_context():
    source = "\n".join([
        ...
        "ignore value source writeEscapedCall type CSignedInt32",
        "return value successCode",
    ])
    proc = run_semsc_source(source, "--run", "--quiet", suffix=".sem")
    expected = (
...
def test_html_template_record_field_holes_infer_root_argument():
    source = "\n".join([
        ...
        "ignore value source writeProfileCall type CSignedInt32",
        "return value successCode",
    ])
    proc = run_semsc_source(source, "--run", "--quiet", suffix=".sem")
    expected = (
...
def test_html_standard_module_import_exposes_hydrate_namespace_and_exports():
    source = "\n".join([
        ...
        "ignore value source writeDocumentCall type CSignedInt32",
        ...
        "ignore value source writeVersionCall type CSignedInt32",
        "return value successCode",
        "",
    ])
...
def test_html_template_long_dynamic_arg_is_bounded_and_terminated():
    ...
    source = "\n".join([
        ...
        "run writeLongCall",
        "ignore value source writeLongCall type CSignedInt32",
        "return value successCode",
        "",
    ])
    proc = run_semsc_source(source, "--run", "--quiet", suffix=".sem")
    ...
    cases = [
        ...
        ("unsupported arg type",
         "\n".join([
             ...
             "purpose operation main \"bad html\"",
             "call hydrateBadCall html.hydrate.BadTemplate",
             "argument hydrateBadCall countValue I64 countValue",
             "run hydrateBadCall",
             "return value 0",
         ]),
...
def test_sem_run_trace_emits_agent_jsonl_events():
    src = "\n".join([
        ...
        "async main no",
        "label start",
        "storage module immutable leftValue CSignedInt32 -1",
        "storage module immutable rightValue CSignedInt32 0",
        "storage module immutable successExit ExitCode 0",
        "call negativeCheckCall math.lessThanCSignedInt32",
        "argument negativeCheckCall left CSignedInt32 leftValue",
        "argument negativeCheckCall right CSignedInt32 rightValue",
        "run negativeCheckCall",
        "bind value statusIsNegative Bool negativeCheckCall",
        "branch if condition statusIsNegative target returnSuccess",
...
def test_sem_explain_crash_reports_runtime_panic_context():
    ...
    src = "\n".join([
        ...
        "async main no",
        "label start",
        "storage module immutable numeratorValue CSignedInt64 7",
        "storage module immutable zeroDivisor CSignedInt64 0",
        "call divideByZeroCall math.divideI64",
        "argument divideByZeroCall left CSignedInt64 numeratorValue",
        "argument divideByZeroCall right CSignedInt64 zeroDivisor",
        "run divideByZeroCall",
        "bind value quotientValue CSignedInt64 divideByZeroCall",
...
def test_codegen_diagnostic_is_agent_readable():
    src = "\n".join([
        ...
        "argument badCall left I64 missingValue",
        "argument badCall right I64 1",
        "run badCall",
        "bind value resultValue CSignedInt64 badCall",
...
def test_web_codegen_rejects_unsupported_http_target():
    src = "\n".join([
        ...
        "output operation jsonHandler CSignedInt32",
        ...
        "storage module immutable okStatus CSignedInt32 200",
        "storage module immutable jsonBody CNullTerminatedByteString \"{}\"",
        "call jsonWriteCall http.responseJson",
        "argument jsonWriteCall response HttpResponse response",
        "argument jsonWriteCall status CSignedInt32 okStatus",
        "argument jsonWriteCall body CNullTerminatedByteString jsonBody",
        "run jsonWriteCall",
        "bind value responseStatus CSignedInt32 jsonWriteCall",
        "return value responseStatus",
    ])
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_sqlite_codegen_rejects_unsupported_target():
    ...
    src = "\n".join([
        ...
        "label start",
        "storage module immutable dbPath CNullTerminatedByteString \":memory:\"",
        "call unsupportedSqliteCall sqlite.notARealEntryPoint",
        "argument unsupportedSqliteCall path CNullTerminatedByteString dbPath",
        "run unsupportedSqliteCall",
        "bind value unsupportedSqliteResult CSignedInt32 unsupportedSqliteCall",
...
def test_json_body_record_literal_runs_from_constant():
    source = "\n".join([
        ...
        "argument writeCount value I64 countValue",
        "run writeCount",
        "ignore value source writeCount type CSignedInt32",
        "return value 0",
        "",
    ])
...
def test_json_record_stringify_parse_round_trip_runs_end_to_end():
    source = "\n".join([
        ...
        "argument writeTitle text CNullTerminatedByteString parsedTitle",
        "run writeTitle",
        "ignore value source writeTitle type CSignedInt32",
        "fieldGet parsedCount I64 parsed count",
        "call writeCount console.writeIntegerLine",
        "argument writeCount value I64 parsedCount",
        "run writeCount",
        "ignore value source writeCount type CSignedInt32",
        "return value zero",
        "",
