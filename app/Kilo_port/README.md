...
This directory contains a SemanticScript port of antirez/kilo plus an
unmodified upstream clone for comparison.

## Upstream

The upstream C implementation is cloned locally at:

```text
app/Kilo_port/upstream-kilo/
```
...
Keep `upstream-kilo/` unmodified. Port work lives beside it in
`build.sem`, `main.sem`, and generated artifacts under `build/`.
...
app/Kilo_port/
  ...
  upstream-kilo/     # unmodified upstream C baseline
...
```text
app/Kilo_port/build/kilo_port.exe
```
...
```powershell
cd C:\Users\Cam\Desktop\AgentScript
.\app\Kilo_port\build\kilo_port.exe scratch.txt
```
...
Use this only to compare behavior against the original C implementation:

```powershell
cd C:\Users\Cam\Desktop\AgentScript\app\Kilo_port\upstream-kilo
make
