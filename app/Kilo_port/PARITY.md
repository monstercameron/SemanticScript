...
- Upstream source: `app/Kilo_port/upstream-kilo/`
...
- Port target: `app/Kilo_port/build.sem` and `app/Kilo_port/main.sem`
...
- [x] `python SemanticScript\compiler\semsc.py app\Kilo_port\build.sem --parse-only --lint --quiet`
  completes.
- [x] `python SemanticScript\compiler\semsc.py app\Kilo_port\build.sem --emit-exe --quiet`
  emits `app/Kilo_port/build/kilo_port.exe`.
...
- [x] Repeatable smoke coverage lives in `app/Kilo_port/scripts/test_kilo_port.py`.
