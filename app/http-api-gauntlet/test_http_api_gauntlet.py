    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if (
                status == 200
                and payload == "ok\n"
                and headers.get("x-gauntlet-app") == "http-api-gauntlet"
            ):
                return
...
def assert_common_headers(headers, path):
    ...
    assert headers.get("x-gauntlet-app") == "http-api-gauntlet", headers
...
def main():
    with tempfile.TemporaryDirectory(prefix="ss_http_gauntlet_") as temp_dir:
        ...
        try:
            ...
            assert header_missing_response.get("x-gauntlet-app") == "http-api-gauntlet", header_missing_response
            assert_response(
            ...
            assert required_missing_headers.get("x-gauntlet-app") == "http-api-gauntlet", required_missing_headers
